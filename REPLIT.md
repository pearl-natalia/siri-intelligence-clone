...
On first account access, the app creates only its own `swift_web_users`, `swift_web_sessions`, `swift_web_chats`, `swift_web_memories`, and `swift_web_memory_settings` tables. Sessions expire after 24 hours, store a hashed random token server-side, and use Secure / HttpOnly / SameSite cookies. All chat reads and writes filter by the verified session's user; request headers and user-supplied IDs never establish identity. Authenticated mutations require CSRF tokens. Saved replies use server-owned history and revisions reject overlapping updates. The browser demo limits each account to 100 chats, with 100 turns per chat.
